...
date_specification <- paste0(date, "_", source) # Two runs where conducted on this date
folder_path <- paste0(here("lab_test", "data", date_specification))
...
output_folder <- paste0(here("clean_data", date, "_clean_air"))
...
#####################################
#### Align SMPS & APS: PM (mass) ####
#####################################

# aligned_mass <- align_instruments(SMPS_data, APS_data)
# assign(paste0("aligned_mass_", date), aligned_mass)
# save(list = paste0("aligned_mass_", date), file = paste0(output_folder, "/aligned_mass_", date, ".rda"))
