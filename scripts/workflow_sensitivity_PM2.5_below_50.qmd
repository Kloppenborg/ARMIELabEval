...
figure1_long <- test |> select(timestamp_rounded_5min, source, sensor, SPS30_PM2.5, PM25_dusttrak, PM1_SMPS_SPS30, PM25_APS_valid) %>%
  mutate(
...
# A new table 1 with number and mass concentration from SMPS + APS and mass concentration from DustTrak. with mean, median, range and SD. grouped on source

```{r}
# Build grouped GT table
table_1 <- figure1_long  |>
...
##################################################
table1_data_clean <- by_sensor_data |>
  filter(PM2.5_ref >=5) |> 
  mutate(
...
#Bland-altman plots for supplementary. These plots are faceted by source and method, with LoA and mean error calculated per source using the by_sensor data
...
p_box_RE <- boxplot_data |>
  ggplot(aes(x = source, y = relative_error, fill = PM25_group)) +
  geom_boxplot(outlier.alpha = 0.3) +
  facet_wrap(reference ~ method, scales = "fixed") +
  scale_fill_manual(
    values = c("PM2.5 =< 5 µg/m³" = "#009E73", "PM2.5 > 5 µg/m³" = "#CC79A7")
  ) +
  geom_hline(yintercept = 0, color = "gray40", linewidth = 0.8, linetype = "solid") +
  coord_cartesian(ylim = c(-200, 600)) +
  theme_minimal() +
  theme(
    panel.spacing = unit(1.7, "lines"),        # Increase space between facets
...
table2_data_clean_air <- by_source_data |>
...
table2_data <- bind_rows(table2_data, table2_data_clean_air)
...
table_2 <- table2_data |>
  ...
  arrange(source, reference, method) |>
  gt(groupname_col = "source") |>
  tab_header(title = "Table 2: Performance metrics based on the leave-one-source-out approach") |>
  cols_label(
    reference  = "Comparison",
    method     = "Method",
    r          = "Pearson's r",
