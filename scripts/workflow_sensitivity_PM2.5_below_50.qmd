---
title: "Sensitivity analysis of workflow with ARMIE readings below 50 ug/m3 and above 5 ug/m3"
...
folders <- c(
  "Sensitivity",
  "Sensitivity/plots",
  "Sensitivity/plots/supplementary",
  "Sensitivity/tables",
  "Sensitivity/tables/supplementary"
)
...
all_data <- all_data |> filter(SPS30_PM2.5 <= 50 & SPS30_PM2.5 >= 5) 
```
...
p_uncal_AE <- by_sensor_data |>
  ...
  facet_wrap(~source) +
  labs(
    title = expression("ARMIE " * PM[2.5] * " vs Reference PM2.5 Concentrations"),
    subtitle = "Uncalibrated values from the ARMIE sensor",
...
p_cal_AE <- by_sensor_data |>
  ...
  labs(
    ...
    subtitle = "Calibrated values from the ARMIE sensor",
    x = expression("Reference " * PM[2.5] * " (µg/m³)"),
    y = expression("ARMIE " * PM[2.5] * " (µg/m³)"),
    color = "Comparison"
  ) +
...
p_scatter
# Save them
ggsave("plots/supplementary/scatter.png", plot = p_scatter, width = 8, height = 8, dpi = 600, bg = "white")
```

plot figure RE_scatter with relative error between ARMIE and comparison instrument (For supplementary)
...
# Save them
ggsave("plots/supplementary/RE_scatter.png", plot = p_scatter_RE, width = 8, height = 8, dpi = 600, bg = "white")
...
BA_plot
ggsave("plots/blandaltman_figure2.png", plot = BA_plot, width = 10, height = 10, dpi = 600, bg = "white")
```
...
ggsave("plots/figure3_RE_boxplot_combined.png", plot = p_box_RE, width = 10, height = 10, dpi = 600, bg = "white")
```
...
# Generate Table 3 summarizing mean and sd from Bland Altman plot with loso by source data
...
ggsave("plots/figure4_bland_altman_source_specific_Loa_by_source.png", plot = BA_plot_source, width = 8, height = 12, dpi = 600, bg = "white")
```
