...
# Scatter plots with ARMIE PM2.5 readings and comparision instruments (For supplementary)
...
p_uncal_AE <- by_sensor_data |>
  ...
  facet_wrap(~source) +
  labs(
    # title = expression("ARMIE " * PM[2.5] * " vs Reference" * PM[2.5] * " Concentrations"),
    subtitle = "Uncalibrated values from the ARMIE node",
  x = expression("Reference " * PM[2.5] * " (µg/m"^3 * ")"),
  y = expression("ARMIE " * PM[2.5] * " − Reference " * PM[2.5] * " (µg/m"^3 * ")"),
  color = "Comparison") +
  theme_bw() +
  scale_color_manual(values = c(
    "DustTrak" = "#E69F00",
...
p_cal_AE <- by_sensor_data |>
  ...
  labs(
    # title = expression("ARMIE " * PM[2.5] * " vs Reference" * PM[2.5] * " Concentrations"),
    subtitle = "Calibrated values from the ARMIE node",
  x = expression("Reference " * PM[2.5] * " (µg/m"^3 * ")"),
  y = expression("ARMIE " * PM[2.5] * " − Reference " * PM[2.5] * " (µg/m"^3 * ")"),
  color = "Comparison") +
  theme_bw() +
...
p_uncal_RE <- by_sensor_data |>
  ...
  facet_wrap(~source) +
  labs(
    # title = expression("Relative error of ARMIE " * PM[2.5] * " vs Reference " * PM[2.5]),
    subtitle = "Uncalibrated values from the ARMIE node",
    x = expression("Reference " * PM[2.5] * " (µg/m³)"),
    y = "Relative error (%)",
    color = "Comparison"
  ) +
  theme_bw() +
...
p_scatter_RE <- (p_uncal_RE | p_cal_RE) +
  plot_layout(guides = "collect") + 
  plot_annotation(title = expression("Relative error of ARMIE " * PM[2.5] * " vs Reference " * PM[2.5])) &
  theme(legend.position = "bottom")
...
BA_plot_uncal <- bland_altman_data |>
  ...
  x = expression("DustTrak " * PM[2.5] * " (µg/m"^3 * ")"),
  y = expression("ARMIE " * PM[2.5] * " − DustTrak " * PM[2.5] * " (µg/m"^3 * ")"))+
  theme_bw()


BA_plot_cal <- bland_altman_data |>
  filter(reference == "SMPS + APS") |>
  ggplot(aes(x = ref_val, y = diff_val, color = source)) +
  geom_point(alpha = 0.4) +
  geom_smooth(aes(group = 1), method = "lm", color = "black", se = TRUE, fullrange = TRUE) +
  geom_hline(aes(yintercept = mean_diff), color = "#E69F00") +
  ...
  labs(
  # subtitle = "Uncalibrated",
  x = expression("SMPS + APS " * PM[2.5] * " (µg/m"^3 * ")"),
  y = expression("ARMIE " * PM[2.5] * " − SMPS + APS " * PM[2.5] * " (µg/m"^3 * ")"))+
  theme_bw()
...
ggsave("plots/blandaltman_figure2.png", plot = BA_plot, width = 10, height = 10, dpi = 1000, bg = "white")
```
...
bland_altman_plot_spec_loa_uncal <- bland_altman_data_spec_loa %>%
  ...
  geom_hline(aes(yintercept = mean_diff), linetype = 1) +
  geom_hline(aes(yintercept = mean_diff + 1.96 * sd_diff), linetype = 3) +
  geom_hline(aes(yintercept = mean_diff - 1.96 * sd_diff), linetype = 3) +
  geom_hline(yintercept = 0, linetype = 2) +
  xlim(0, 400) +
  ...
  theme(
    ...
    strip.text       = element_blank(),        # hide source labels on LEFT plot
    plot.subtitle    = element_text(hjust = 0.5)
  ) +
  labs(
    subtitle = "Uncalibrated",
    x = expression("Reference " * PM[2.5] * " (µg/m³)"),
    y = expression("ARMIE " * PM[2.5] * " − Reference " * PM[2.5] * " (µg/m³)"),
    color = "Comparison"
  ) +
  scale_color_manual(values = c(
    "DustTrak"   = "#E69F00",
...
bland_altman_plot_spec_loa_cal <- bland_altman_data_spec_loa %>%
  ...
  theme(
    strip.placement  = "outside",
    strip.background = element_blank(),
    strip.text.y.right = element_text(angle = 0), # show source labels once (on RIGHT plot)
    plot.subtitle    = element_text(hjust = 0.5)
  ) +
  labs(
    subtitle = "Calibrated",
...
BA_plot_source_by_sensor <-
  (bland_altman_plot_spec_loa_uncal | bland_altman_plot_spec_loa_cal) +
  plot_layout(guides = "collect") &
  theme(legend.position = "right")

BA_plot_source_by_sensor
# Save them in supplementary
ggsave("plots/supplementary/bland_altman_source_specific_Loa_by_sensor.png", plot = BA_plot_source_by_sensor, width = 8, height = 12, dpi = 600, bg = "white")
```
...
boxplot_data <- by_sensor_data |>
  ...
  mutate(
    method = factor(method, levels = c("Uncalibrated", "Calibrated")),
    PM25_group = if_else(PM2.5_pred <= 5, "=< 5", "> 5")
  )
...
p_box_RE <- boxplot_data |>
  ...
  labs(
    ...
    # subtitle = "Comparison of uncalibrated and calibrated ARMIE measurements",
    x = "Source type",
    y = "Relative error (%)",
    fill = expression("ARMIE " * PM[2.5] * " µg/m³")
  )

p_box_RE
# save the combined boxplot
ggsave("plots/figure3_RE_boxplot_combined.png", plot = p_box_RE, width = 10, height = 10, dpi = 1000, bg = "white")
```
...
p_box_RE_sup
# save the combined boxplot
ggsave("plots/supplementary/RE_boxplot_supplementary.png", plot = p_box_RE_sup, width = 10, height = 10, dpi = 600, bg = "white")
```
...
bland_altman_data_spec_loa_by_source_uncal <- bland_altman_data_spec_loa_by_source %>%
  ...
  labs(
    subtitle = "Uncalibrated",
    x = expression("Comparison " * PM[2.5] * " (µg/m³)"),
    y = expression("ARMIE " * PM[2.5] * " - Comparison " * PM[2.5] * " (µg/m³)"),
    color = "Comparison"
  ) +
  scale_color_manual(values = c(
    "DustTrak" = "#E69F00",
...
ggsave("plots/figure4_bland_altman_source_specific_Loa_by_source.png", plot = BA_plot_source, width = 8, height = 10, dpi = 1000, bg = "white")
```
...
bland_altman_data |>
  ...
  labs(
    title = "Data from Bland-Altman Plot: Uncalibrated ARMIE vs DustTrak",
    x = expression("Reference " * PM[2.5] * " (µg/m³)"), 
    y = "|Difference| (µg/m³)"
  )
...
bland_altman_data |>
  ...
  labs(
    title = "Data from Bland-Altman Plot: Calibrated ARMIE vs DustTrak",
    x = expression("Reference " * PM[2.5] * " (µg/m³)"), 
    y = "|Difference| (µg/m³)"
  )
...
bland_altman_data |>
  ...
  labs(
    title = "Data from Bland-Altman Plot: Uncalibrated ARMIE vs SMPS + APS",
...
bland_altman_data |>
  ...
  geom_smooth(method = "loess", color = "#56B4E9") +
  labs(
    title = "Data from Bland-Altman Plot: Calibrated ARMIE vs SMPS + APS",
    x = expression("Reference " * PM[2.5] * " (µg/m³)"), 
    y = "|Difference| (µg/m³)"
  )
```
...
p_clean_air <- by_sensor_data |>
  ...
  labs(
    title = expression("ARMIE " * PM[2.5] * " vs Reference " * PM[2.5] * " Concentrations"),
    subtitle = "Calibrated values from the ARMIE sensor-node",
    x = expression("Reference " * PM[2.5] * " (µg/m³)"),
    y = expression("ARMIE " * PM[2.5] * " (µg/m³)")
  ) +
  theme_minimal()
...
table4S_gt <- table4S_data_clean |>
  gt(groupname_col = "Source") |>
  tab_header(
    title = "Performance metrics for ARMIE sensor PM₂.₅ concentrations",
    subtitle = "Against reference instruments (SMPS + APS, DustTrak), stratified by source type and calibration method"
  ) |>
  cols_label(
    Reference = "Reference",
